    ...
    {  
        ...
        {  
            ...
            { 
                ...
                int len = 0;  
                char buf[128] = "";  
                char Recv[] = "recv logdata successfully";
                //6.2.1接受客户端数据  
				...
				//先判断第一条是否为对端的请求连接消息
				else if(!strcmp("client is going to send logdata to server" , buf)) 
				{
					close(fd[i]);
					fd[i] = -1;
				}
                else//正常接收到服务器的数据
				{
					...
					while(len > 0)
					{
                    	send(fd[i], Recv, sizeof(Recv), 0);
						len = recv(fd[i] , buf , sizeof(buf) , 0);
						if(len <= 0)  //client端数据传输异常
						{
							close(fd[i]);
							int j;
							for(j = 0;j<3;j++)
